...
def calculate_quadratic_spline(x, y):
    ...
    for i in range(n):
        ...
        step_detail = {
            ...
            'detalles': {
                'x1': x1, 'y1': y1, 'x2': x2, 'y2': y2,
                'calculo_a': f'a = y[{i}] = {y1:.2f}',
                'calculo_b': f'b = ({y2:.2f} - {y1:.2f}) / ({x2:.2f} - {x1:.2f}) - ({h_i:.2f} * (2 * {c_i:.2f} + {c_next:.2f}) / 3) = {b_i:.2f}',
                'calculo_c': f'({y2:.2f} - {y1:.2f}) / {h_i:.2f} - {h_i:.2f} * (2 * {c_i:.2f} + {c_next:.2f}) / 3 = {c_i:.2f}',
                'pendiente_inicial_detalle': f'Pendiente inicial = b = {b_i:.2f}',
                'pendiente_final_detalle': f'Pendiente final = b + 2 * c * (x2 - x1) = {b_i:.2f} + 2 * {c_i:.2f} * ({x2 - x1:.2f}) = {pendiente_final:.2f}',
